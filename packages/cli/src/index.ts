...
async function main() {
  ...
  program
    .name('stereogramer')
    .description('Generate single-image autostereograms (SIRDS) from depth maps')
    .version('0.1.0')
    .option('--sirds', 'Generate a Single Image Random Dot Stereogram (SIRDS)')
    .option('-o, --output <path>', 'Output file path', 'sirds.png')
    .option('-w, --width <pixels>', 'Image width in pixels', '800')
    .option('-h, --height <pixels>', 'Image height in pixels', '600')
    ...
    .option('--dot-scale <scale>', 'Pixel block dimension of dots', '1')
    ...
    .action(async (opts) => {
      try {
        ...
        const outputPath = path.resolve(process.cwd(), opts.output || 'sirds.png');
        ...
        console.log(`Rendering SIRDS (${depthMap.width}x${depthMap.height}, mode: ${mode}, separation: ${separation ?? Math.round(depthMap.width / 8)}px)...`);
